...
REPORT_FILE = "raport_targeted.html"
HISTORY_FILE = "ceny_targeted.json"
DEBUG_JSON_FILE = "debug_targeted.json"
DEBUG_CSV_FILE = "debug_targeted.csv"
DEBUG_XLSX_FILE = "debug_targeted.xlsx"
CSV_FILE = "raport_targeted_excel.csv"
CSV_HISTORY_FILE = "historia_cen_targeted.csv"

# Próg cenowy 600zł
MIN_PRICE_THRESHOLD = 600.00 

# ==========================================
# SCENARIUSZE (TARGETED SNIPER)
# ==========================================
...
# ==========================================
# PARSERY (ADAPTOWANE DO TARGETED)
# ==========================================
...
async def human_scroll(page):
    print("     (Symulacja czytania - przewijanie do stopki...)")
    try:
    ...
    wait_reading = random.uniform(2, 4) 
    print(f"     (Dotarto do końca strony. Pauza {wait_reading:.1f}s)")
...
def export_to_real_excel(all_products):
    ...
    try:
        ...
        try:
            wb.save(DEBUG_XLSX_FILE)
            print(f"--> [NOWY PLIK] Wygenerowano plik Excel: {DEBUG_XLSX_FILE}")
        except PermissionError:
            print(f"\n!!! UWAGA: Plik '{DEBUG_XLSX_FILE}' jest otwarty w Excelu!")
            timestamp = datetime.now().strftime("%H%M%S")
            backup_filename = f"debug_targeted_{timestamp}.xlsx"
            wb.save(backup_filename)
            print(f"--> Zapisano dane w nowym pliku awaryjnym: {backup_filename}")
    except Exception as e:
        print(f"Błąd generowania XLSX: {e}")
...
def export_all_products_to_excel_csv(all_products):
    try:
        ...
        print(f"Błąd generowania Excel Debug CSV: {e}")

# ==========================================
# LOGIKA KATEGORYZACJI (TARGETED)
# ==========================================

def categorize_and_pick_best(all_products):
    CAT_MAP = {
        ("single", 16): "16GB (1x16GB)",
        ("single", 32): "32GB (1x32GB)",
        ("single", 48): "48GB (1x48GB)",
        ("single", 64): "64GB (1x64GB)",
        ("dual", 32): "32GB (2x16GB)",
        ("dual", 48): "48GB (2x24GB)",
        ("dual", 64): "64GB (2x32GB)",
        ("dual", 96): "96GB (2x48GB)"
        ...
        with open(CSV_FILE, 'w', newline='', encoding='utf-8') as f:
            ...
            for cat_name, shops in categorized_data.items():
                for shop_name, product in shops.items():
                    writer.writerow([
                        cat_name, shop_name.upper(), product['title'],
                        f"{product['price']:.2f}".replace('.', ','), 
                        product['link'], datetime.now().strftime("%Y-%m-%d %H:%M")
                    ])
        print(f"--> Wyeksportowano raport do CSV: {CSV_FILE}")
    except Exception as e: print(f"Błąd CSV: {e}")
...
async def main():
    print("--- RAM Sniper TARGETED (Full Force) ---")
    ...
    async with async_playwright() as p:
        ...
        while True:
            try:
                if page.is_closed():
                    print("!!! Okno zamknięte.")
                    break

                print(f"\n[{datetime.now().strftime('%H:%M:%S')}] --- NOWY CYKL CELOWANY ---")
                all_found_products = []
                ...
                for task in SCENARIOS:
                    ...
                    print(f"\n>>> ZADANIE: {shop_name.upper()} [{group_type.upper()}]")
                    ...
                    for i, url in enumerate(links, 1):
                        ...
                        try:
                            ...
                            if shop_name == 'oleole' or shop_name == 'mediaexpert' or shop_name == 'komputronik':
                                try:
                            ...
                            if parser:
                                ...
                                if current_hash in seen_page_hashes:
                                    print(f"     (Duplikat strony [widziana wcześniej]. Koniec zadania.)")
                                    break
                                
                                seen_page_hashes.add(current_hash)

                                if shop_name == 'komputronik' and len(items) < 20:
                                     print(f"     (Mniej niż 20 w Komputronik. Koniec zadania.)")
                                     all_found_products.extend(items)
                                     break

                                print(f"     -> Pobrane: {len(items)}")
                                all_found_products.extend(items)
                            else:
                                print(f"!!! Brak parsera: {parser_name}")
                ...
                if all_found_products:
                    ...
                    generate_pro_html(best_deals, history, "TEST END")
                    export_to_csv(best_deals)
                    
                    print(f"SUKCES. Wszystkie raporty gotowe.")
                    break
                else:
